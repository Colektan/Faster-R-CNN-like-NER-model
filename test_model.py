...
from utils import *
from dataloader import NERDatasetForFRLN
from models import FRLN
...
class TestTrainer():
    def __init__(self, config, device, use_gpu, n_gpu, writer, id2label):
        ...
        self.model = FRLN.from_pretrained(config.model_name_or_path, config=bert_config,
    ...
    def train(self):
        ...
        dataset = NERDatasetForFRLN(self.config, self.tokenizer, mode='train')
        ...
        for epoch in range(int(self.config.num_train_epochs)):
            ...
            if freeze_flag == 0 and epoch >= self.config.freeze_epoch:
                ...
                model_to_save.save_pretrained(os.path.join(self.config.trained_model_path, f'checkpoints_epoch={epoch}'))
                self.tokenizer.save_pretrained(os.path.join(self.config.trained_model_path, f'checkpoints_epoch={epoch}'))
                freeze_flag = 1
                for name, parameter in self.model.bert.named_parameters():
            ...
            for batch, batch_data in enumerate(tqdm(dataloader, desc="Train_DataLoader")):
                ...
                cls_loss, loc_loss, final_cls_loss = self.model(input_ids, label_ids, all_locs, total_locs, total_tags, token_type_ids, attention_mask)
            ...
            self.tokenizer.save_pretrained(os.path.join(self.config.trained_model_path, f'checkpoints_{epoch}'))
    ...
    def test(self):
        ...
        dataset = NERDatasetForCNN(self.config, self.tokenizer, mode='test')
        avg_metrics, cal_indicators, eval_sens = self.evaluate(
...
def main():
    ...
    trainer_bbc = TestTrainer(config, device, use_gpu, n_gpu, writer, id2label)
    trainer_bbc.test()
