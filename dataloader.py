...
class NERDatasetForFRLN(Dataset):
    def __init__(self, config, tokenizer, mode="train"):
        # text: a list of words, all text from the training dataset
        super(NERDatasetForFRLN, self).__init__()
        self.config = config
        self.tokenizer = tokenizer
        if mode == "train":
            self.file_path = config.train_file
    ...
    def __getitem__(self, idx):
        ...
        while len(data_text_list) < max_seq_length:
            data_text_list.append(' ')
            data_label_list.append(' ')

        features.data['label_ids'] = label_ids
        # features.data['data_text_list'] = data_text_list
        # features.data['data_label_list'] = data_label_list

        all_locs = torch.zeros((max_seq_length, 3), requires_grad=False)
        judge = -1
        for num, idx in enumerate(label_ids):
            if idx in [-1, 5, 7, 8]:
                judge = -1
            else:
                all_locs[num, 0] = 1.0
                if idx in [3, 4, 6]:
                    ...
                    all_locs[num, 1] = float(start - num)
        judge = -1
        for num, idx in enumerate(reversed(label_ids)):
            if idx in [-1, 5, 7, 8]:
                judge = -1
            elif judge == -1:
                if idx not in [3, 4, 6]:
                ...
                all_locs[max_seq_length - num - 1, 2] = float(num - end)
            elif idx == judge[1]:
                all_locs[max_seq_length - num - 1, 2] = float(num - end)
                judge = -1
        
        total_locs = torch.zeros((max_seq_length, max_seq_length), requires_grad=False)
        total_tags = torch.zeros((max_seq_length), dtype=torch.long, requires_grad=False)
        judge = -1
        pos = 0
        for num, idx in enumerate(label_ids):
            if idx in [3, 4, 6]: 
                if judge != -1:
                    ...
                    total_tags[pos] = tag
                    total_locs[pos, :] /= (end - start + 1)
                    pos += 1
                total_locs[pos, num] = 1.0
                start = num
                if idx == 3:
                    tag = 3
                    judge = 2
        ...
        features.data['all_locs'] = all_locs
        features.data['total_tags'] = total_tags
        features.data['total_locs'] = total_locs
...
class ClueNERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = self.data[idx]
        data_text_list = data['text']
        word_areas = torch.zeros((max_seq_length, 3))
        total_tags = torch.zeros((max_seq_length), dtype=torch.long)
        total_areas = torch.zeros((max_seq_length, max_seq_length))

        pos = 0
        for k in data['label'].keys():
            for v in data['label'][k].values():
                start = v[0][0]
                end = v[0][1]
        ...
        features['all_locs'] = word_areas
        features['total_tags'] = total_tags
        features['total_locs'] = total_areas
